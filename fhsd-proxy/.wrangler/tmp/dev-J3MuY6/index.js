...
// .wrangler/tmp/bundle-cXBhlB/checked-fetch.js
...
__name(parseContest, "parseContest");
...
function precinctHashString(precincts) {
  return Object.keys(precincts).sort().map(
    (p) => `${p}:${precincts[p].total_votes}`
  ).join("|");
}
__name(precinctHashString, "precinctHashString");
...
var src_default = {
  async fetch(request, env) {
    ...
    const hashInput = precinctHashString(fhsdMap) + (propRT ? "||" + precinctHashString(propRT.precincts) : "");
...
// .wrangler/tmp/bundle-cXBhlB/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-cXBhlB/middleware-loader.entry.ts
